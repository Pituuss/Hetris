...
isNotColision state = if (\(a,b) -> b) (blockPos state) < 0 then True else 
  if bottomWallColision (blockCoordList state) || mapColision state (blockCoordList state) then False else True
...
loadNewState state =
  state {blockPos = (7,0),gameBoard = renderBlock (block state) (blockPos state) (gameBoard state) }

  
mapColision :: State -> [(Float,Float)] -> Bool
mapColision state [] = False
mapColision state x = pointColision (head x) (numberRows(gameBoard state)) || mapColision state (tail x) 

pointColision :: (Float,Float) -> [(Float, Row)] -> Bool
pointColision (pointx,pointy) x = if (\(a,b) -> a) (head x)  == pointy then colision pointx  (numberCells ((\(a,b) -> b) (head x)))
  else pointColision (pointx,pointy) (tail x)
  
colision :: Float -> [(Float, Cell)] -> Bool
colision yx x = if yx == (\(a,b) -> a) (head x) then check else colision yx (tail x)
  where check = if cellColor ((\(a,b) -> b) (head x)) == black then False else True
